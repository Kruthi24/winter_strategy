...
detectability_file = "results/"+trial+"_detectability_prob_ranked.csv"
outdir = "results/"+trial+"_prob_ranked/"
...
def process_inj(j):
    ...
    for i in range(nsites):
        ...
        tileObj = ranked_tiling.RankedTileGenerator(skymapfile=path+fname,
                                                    configfile=config_dir+site_names[i]+"_config.ini", outdir=outdir)
        ranked_tiles = tileObj.getRankedTiles(resolution=resolution, save_csv=True, tag=tag)
        ranked_tile_saved_csv = outdir+tag+"_ranked_tiles.csv"
        ...
        observing = scheduler.Scheduler(ranked_tiles_csv = ranked_tile_saved_csv, configfile=config_dir+site_names[i]+"_config.ini",
        ...
        schedule = observing.observationSchedule(duration=duration, eventTime=inj_trigger_gps,
                                                ...
                                                save_schedule=True, tag=tag, latency = latency)
